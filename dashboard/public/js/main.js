...
function formatUserField(state) {
  ...
  var $state = $(
    '<div class="student" id="' +
      ...
      '</div>\
			<div class="timestamp">' +
      $(state.element).data('timestamp') +
      '</div>\
		</div>',
  );
  new icon().load(
    '/public/img/owner-icon.svg',
...
function formatColorField(state) {
  ...
  var id =
    $(state.element).data('stroke') +
    $(state.element).data('fill') +
  ...
  if ($(state.element).data('icon')) {
    new icon().load(
      '/public/img/' + $(state.element).data('icon') + '.svg',
      JSON.parse($(state.element).val()),
      id,
    );
  } else {
    new icon().load(
        ...
        if (response.data.datasets[0].data.length == 0) {
          var html =
            '<div class="text-center">\
											...
											<p data-l10n-id="noGraphDataText">' +
            document.webL10n.get('noGraphDataText') +
            '</p>\
										</div>';
